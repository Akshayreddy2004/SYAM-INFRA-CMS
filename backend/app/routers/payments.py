...
def update_payment_history(history_id: int, payment: schemas.PaymentHistoryUpdate, db: Session = Depends(deps.get_db), current_user: models.User = Depends(deps.get_current_user)):
    history = db.query(models.PaymentHistory).filter(models.PaymentHistory.id == history_id).first()
    if not history:
        raise HTTPException(status_code=404, detail="Payment record not found")
        
    schedule = db.query(models.PaymentSchedule).filter(models.PaymentSchedule.id == history.schedule_id).first()
    if schedule and payment.amount is not None:
        # Revert old amount and add new amount
        schedule.amount_received = schedule.amount_received - history.amount + payment.amount
    ...
    if payment.payment_date is not None:
        history.payment_date = payment.payment_date
...
@router.put("/{schedule_id}/legacy")
def update_legacy_payment(schedule_id: int, payload: schemas.PaymentHistoryUpdate, db: Session = Depends(deps.get_db), current_user: models.User = Depends(deps.get_current_user)):
    schedule = db.query(models.PaymentSchedule).filter(models.PaymentSchedule.id == schedule_id).first()
    if not schedule:
        raise HTTPException(status_code=404, detail="Schedule not found")
        
    if payload.amount is not None:
        schedule.amount_received = payload.amount
        if schedule.amount_received <= 0:
            schedule.amount_received = 0
            schedule.status = "Pending"
        elif schedule.amount_received >= schedule.expected_amount:
